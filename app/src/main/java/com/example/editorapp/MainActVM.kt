...
class MainActVM : ViewModel() {
    ...
    val images = arrayOf((R.drawable.aim_filter).toString(), (R.drawable.film_filter).toString(),
    ...
    var filterLocation : String = images[0]

    fun getChosenFilter (appContext : Context) : Bitmap
    {
        val imageGetter = RetrieveImageHandler(appContext)
        return imageGetter.getBitmapFromResources(filterLocation.toInt())
    }

    fun setChosenFilter (appContext : Context, position : Int, height : Int, width : Int) : Bitmap
    {
        val imageGetter = RetrieveImageHandler(appContext)
    ...
    fun sendCapturedImage(appContext: Context, file : File) : Intent
    {
        val goto = Intent(appContext, ImagePreviewActivity::class.java)
